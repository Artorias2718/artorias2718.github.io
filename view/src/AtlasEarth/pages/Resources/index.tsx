...
import useGetResources from "../../api/queryHooks/resources/useGetResources.ts";
            ...
            <Grid item>
                ...
                <ul>
                    { data && data.data.map((item, id) => {
                        return (
                            <li key={id}>
                                <MuiLink component={RouterLink} style={styles.link} to={item.href} target='__blank'>{item.label}</MuiLink>: {item.description}
                            </li>
                        );
                    })}
                </ul>
